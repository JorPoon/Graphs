...
#traverse using dfs to go through each room that has a question mark
...
# once room visited == the length of total rooms in graph then it returns 
...
#     graphed_rooms[room] = directions
# print(graphed_rooms)



def mapTraversal(player):
    #stops when length of graphed room == roomGraphs
    if len(graphed_rooms) == len(roomGraph):
        return "No more rooms"
    #where the player starts
    current_room = player.currentRoom.id

    #store current room into graph with available exits
    if current_room not in graphed_rooms:
        graphed_rooms[current_room] = {}
        for direction in player.currentRoom.getExits():
            graphed_rooms[current_room][direction] = "?"
    print(graphed_rooms)
    for exits in player.currentRoom.getExits():
        if graphed_rooms[current_room][exits] == "?":
            prevRoom = player.currentRoom.id
            traversalPath.append(exits)
            player.travel(exits)
            graphed_rooms[prevRoom][exits] = player.currentRoom.id
    ...
    # print(f"Length: {len(graphed_rooms)}")
