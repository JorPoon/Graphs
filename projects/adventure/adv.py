...
#starting vertex is player's current room
#traverse using dft to go through each room that has a question mark
#store the rooms being visited by replacing the question marks
# if you hit a dead end, go back to the first room that still contains a question mark.
# increment length of room visited by 1 everytime a new room is stored
# once room visited == the length of total rooms in graph then it is done 

#build graph and add rooms as vertex with 4 directions containing question marks
...
def mapTraversal(player, move=''):
    ...
    #Ends the loop when length of graph rooms equals the length of the rooms
    if len(graph_rooms) == len(roomGraph):
        return


    currentRoom = player.currentRoom.id

    # Creates new indices as we travel dft first
    if player.currentRoom.id not in graph_rooms:
        graph_rooms[player.currentRoom.id] = {}
        for exit in player.currentRoom.getExits():
            graph_rooms[player.currentRoom.id][exit] = '?'

    if move is not '':
        opposite = opposite_directions[move]
        last_Room = player.currentRoom.getRoomInDirection(opposite)
        graph_rooms[currentRoom][opposite] = last_Room.id

    new_move = '?'

    #  travel in that direction of unexplored path
    for exit_rooms in player.currentRoom.getExits():
        if graph_rooms[currentRoom][exit_rooms] == '?':
            
            new_move = exit_rooms
            # move player 
            player.travel(exit_rooms)
            # Append the current exit to the traversal path for test
            traversalPath.append(exit_rooms)
            # Set the new room to the current room 
            new_room = player.currentRoom.id
            graph_rooms[currentRoom][exit_rooms] = new_room
            mapTraversal(player, exit_rooms)
            break

    # find the nearest unexplored exit using BFS
    shortest_path = []
    ...
    if new_move == '?':
        ...
        while q.size() > 0:
            ...
            if currentRoom not in visited_rooms:
                ...
                if '?' in graph_rooms[currentRoom].values():
                    shortest_path = path
                    q = Queue()
                    break

                # For every direction in the current room, add it to the path to search through and add it to the queue
                for neighbor in graph_rooms[currentRoom].values():
                    new_path = list(path)
                    new_path.append(neighbor)
                    q.enqueue(new_path)
    
    #converting room id to cardinal directions
    for to_room in shortest_path:
        # print(to_room)
        room = player.currentRoom.id
        graph = graph_rooms[room].keys()
        # print(graph)

        for cardinal in graph:
            # print(cardinal)
            if graph_rooms[room][cardinal] == to_room:
...
# def bfs_map(starting, destination):
#     travel_path = [] 
#     bfq = Queue()
#     sbf = set()
#     bfq.enqueue([starting])
...
#         if end not in sbf:
#             sbf.add(end)
#             if '?' in graphed_rooms[end].values():
#                 travel_path = bfq_path
#                 q = Queue()
#                 break

#             for neighbor in graphed_rooms[end].values():
#                 copy = list(bfq_path)
...
#                 bfq.enqueue(copy)
    ...
    #     if next_room not in visted and next_room in roomGraph:
    #         for neighbor in roomGraph[next_room]:
    #             path_new = path[:]
    #             path_new.append(neighbor)
    #             stack.push(path_new)
    #             if neighbor not in roomGraph:
    #                 graphed_rooms[neighbor] = directions
    #         visted.add(next_room)
